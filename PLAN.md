...
| 2026-09-18 | MCP-only AST-скан: агент мог инициировать построение AST-базы только гибридно — `project_scan` через MCP падал по 60-с таймауту MCP-клиента на больших репо, и скилл учил досматривать результат через CLI/логи/БД. Вынесено в фоновый job (`src/project/scanjob.rs`: scan → god nodes → векторизация, spawn_blocking); `project_scan` ждёт до 45 с (`wait=false` — мгновенный ack), новый MCP-инструмент **`project_scan_status`** (34-й) отдаёт job-статус и сводку из БД — CLI больше не нужен агенту. Повторный запуск по идущему скану идемпотентен. CLI `project scan` не тронут (скриптовые запуски) | контракт +1 инструмент, CHANGELOG; менеджер jobs — in-process (жизнь = процесс сервера), персистентность не нужна: статус нужен только во время скана |
