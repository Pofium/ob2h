...
| 2026-09-13 | Заказ v1.3: трек A — прокачка ядра памяти (golden set + `ob2h bench`, честный prefetch, trust/feedback-петля, int8-квантование и бэкапы, надёжность эмбеддингов/плагина); трек B — Ralph Knowledge Layer (циклы разработки, авто-вердикты, staleness, скилл `ralph-loop`). Детальный план — `PLAN_v1.3.md`; Ralph-спека — `Ralph Knowledge Layer (ob2h)_v1.2.3.md`. Ralph-миграция переименована: в спеке «M4», фактически **M6** (M4 занята project_files v1.2, M5 — trust/memory_links v1.3) | аудит recall-пути (`build_context` без гибрида, мёртвые `touch_access`/`memory_relations`), размер БД 939 МБ, молчащий FakeEmbedding-фолбэк; интернет-исследование технологий 2026-09-13 (§8 плана) |
| 2026-09-13 | v1.3 Фазы 21–22 реализованы: `ob2h bench` (golden 36 кейсов, recall@k/MRR/латентность, baseline `docs/bench_baseline.md`); честный prefetch — гибрид в `build_context`, скоринг с recency/access, MMR λ=0.7, бюджет `OB2H_PREFETCH_MAX_CHARS=8000` по record-границам, touch вошедших; `memory_context` + `max_chars` (запись в CHANGELOG). Context-bench 0.833→0.542 — падение против смещённого importance-baseline, разобрано в baseline-доке; search не изменился. Также: security-фикс path-traversal в workspace_read/write (до плана, по требованию pre-commit-хука) | bench-гейт ADR-13 введён до твиков формул; смещение baseline (golden из high-importance пула) задокументировано |
| 2026-09-13 | v1.3 Фаза 23 реализована (миграция M5 → схема v6): memories.trust/last_feedback_at, memory_links; touch→trust+0.02, decay→trust, `memory_feedback` (№26), dream-ревизия (confirmed/outdated/contradicted → +0.1/−0.4/−0.5, флаг OB2H_DREAM_MEMORY_REVISION, дефолт on), candidate_for_forget<0.15 без автоудалений, автосвязи same_project/category + related=true + каскад при forget, дедуп-правило builtin в промпте дрима. Отступление от 23.5: entity-связи через OneKE-lite на каждом save НЕ делаем (токены на каждый ход), kind=entity зарезервирован — вынести в dream-extract/бэклог | правило «никаких автоудалений»; LLM-сигнал trust — bounded дельты, выключаемо |
| 2026-09-13 | v1.3 Фаза 24 реализована: int8 v2 (magic 0x01 + scale — длины блоба недостаточно: 388=4×97), dual-read, все писатели на v2; `ob2h db quantize-embeddings` (пре-бэкап+VACUUM); `ob2h backup --scope quick|full` + `verify`; раздельная ротация. Квантование проверено на копии живой БД: 963→619 МБ, recall идентичен (гейт ADR-13 пройден); на живой БД — после рестарта на новый бинарник. Находка: ~269K узлов графа с пустыми X''-эмбеддингами (проекция −670 МБ завышена); счётчик легаси исправлен (length>0) | не квантовать живую БД до деплоя нового бинарника (старый процесс f32-v2 не видит); пустые блобы — кандидат на чистку в maintenance |
| 2026-09-13 | v1.3 Фаза 25 реализована: громкий FakeEmbedding (runtime-бэкенд в omnes_stats, warn в memory_search при деградации, doctor — реальная канареечная проверка с красным статусом); плагин `sync_turn(turn_author=...)` → meta.author в daily-записях, `memory_context`+author фильтрует чужие записи (мультиагентные gateway-сессии); LWW-счётчик в `sync status`; контракт session_log/session_ingest/memory_context + author (аддитивно). 25.3 (реранкер) отложена — ort-зависимость требует отдельного решения (ADR-14 §8). Python-контракт-тест обновлён (26 тулов), TestLlmChildEnv изолирован от живого ключа машины. Коммит селективный: в рабочем дереве параллельная сессия ведёт Ф30 PLAN_v1.4 (bench_gate) — её WIP не коммитился | multi-авторные gateway-сессии смешивали авторов; деградация эмбеддингов была невидима |
| 2026-09-13 | v1.3 Фазы 26–27 (Ralph-ядро) реализованы: миграция M6 (ralph_runs/ralph_iterations/ralph_findings/ast_changes; graph_nodes не тронут, confidence сохранена — тест схемы); сервис `src/ralph/` + 7 MCP-инструментов (ralph_start/iteration/verdict/context/report, ast_diff/ast_history → контракт 34 с memory_merge Ф31.2); авто-вердикты только по tests_summary (ADR-K4), идемпотентность (run,task,n), AST-рескан с дельтой по сигнатурам, staleness-pass по findings.symbols, контекст-пакет (спека/негатив/reuse-кандидаты из AST/инварианты/god nodes, lite|full, выгрузка на диск + context_ref), debt-леджер (deferred/no-trigger), gain: reuse-hit rate. Ф28 (dream-фаза 3, скилл ralph-loop, CLI findings-to-memory) — следующей, файлы dream/cli были заняты параллельной сессией | параллельная сессия вела Ф30–31 PLAN_v1.4 — коммит селективный, memory_merge Ф31.2 включён с атрибуцией; reuse-семпоиск отложен в Ф33 v1.4 |

| 2026-09-13 | v1.3 Фаза 28 реализована: dream-фаза 3 — ревизия stale-findings LLM по свежей истории (reverified → verified source=dream; OB2H_DREAM_RALPH, дефолт on); скилл `skills/ralph-loop/SKILL.md`; CLI `ob2h ralph findings-to-memory [--dry-run]` (FR-K12); doctor — сироты-раны >7 дней; `omnes_stats` — блок ralph; SYNC.md — ralph_* вне бандлов (ADR-K6); CHANGELOG — записи Ф25–28. 28.5 (benchmark) и 28.6 (живой e2e) — за владельцем | файлы dream/cli освободились после коммитов параллельной сессии |
| 2026-09-13 | v1.3 Ф29 закрыта полностью: параллельная сессия сделала PPR по памяти (631a5bf: memory_search mode=graph, graph_reason scope=memory); дополнено — PPR по графу знаний: `GraphService::ppr_search` (сиды из матчинга, веса рёбер по типу OB2H_PPR_WEIGHTS, проектный фильтр), `graph_search mode=ppr`, `graph_reason scope=all` блок `ppr_docs:`; тест multi-hop alpha→beta→gamma (1-hop не достигает gamma) + изолированный узел без массы. Квантование живой БД выполнено: 964,6→617,4 МБ (−36%), verify пре-бэкапа ok, bench без деградации | 29.2/29.3 были закрыты только для памяти — multi-hop по графу знаний отсутствовал; деградация recall после квантования — ноль |
| 2026-09-13 | v1.4 ядро Ф30–35 реализовано (параллельная сессия, e27a738…8432c8e): ночной bench-гейт дрима (dual-сигнал recall@5/MRR + workspace-инвариант, dream_restore, OB2H_BENCH_GATE, история data/bench/history.jsonl); консолидация Ф31 — save-дедуп (cos 0.98 identity / 0.75 merge_candidate), офлайн LLM-вердикты merge/keep_both/contradicts/supersedes (MELD), compaction → hmem-digest/*, `memory_merge` №34, CLI `memory dedup`; typed edges Ф32 (M7: memory_links.deleted_at, contradicts/supersedes/causes за флагом OB2H_DREAM_BELIEF, conflict-разметка `[conflicts]`); PPR по памяти Ф33 (memory_search mode=graph, graph_reason scope=memory\|all, dual-seed, OB2H_PPR_WEIGHTS); sync v2 Ф34 (дельты по курсору, поле-уровневый merge + conflicts.jsonl + OB2H_SYNC_KEEP_LOSERS, trust/links в бандле, `sync verify`); Ф35 — ADR-35.1 (vec0-эксперимент: recall@10 1.0, 2.9× на векторе, но p95<50 мс не достигнут — узкое место лексический скан графа; OB2H_VEC0 opt-in), MCP-аннотации, ретеншн логов, bench history + вердикт 35.2 | закрытие бэклога §8 PLAN_v1.3 по данным nightly-bench; без ломки ADR-1..8 |
| 2026-09-13 | Трек C Ф36–40 (coding graph) реализован: `project_call_path` №35 + callers/callees + dead-code (CLI `project dead-code`), repo-map под token budget (PPR по file-dependency + binary-fit, `project_context mode=repo_map`), edit-time blast radius (warn-only, `OB2H_EDIT_BLAST`, MCP-ресурс), type-resolve lite + provenance M8 (EXTRACTED/RESOLVED/INFERRED), communities (label propagation) + framework-рёбра ROUTE/QUERIES_TABLE + связка memory↔code (meta.code_symbols, with_memory) | контракт 35 инструментов; каждая фаза самостоятельна, вливалась по мере готовности |
| 2026-09-13 | Аудит PLAN_v1.3 (выполнен отдельной сессией) и подготовка релиза 1.4.0. Находки и решения: (1) trust так и не участвовал в скоринге build_context (константа 0.1 с устаревшим комментом) — §22.2 реализовано, но вес вынесен в `OB2H_CONTEXT_TRUST_WEIGHT` с дефолтом **0**: bench живой БД показал recall@5 0.569→0.417 (−27%, гейт ADR-13) на молодой trust-статистике (смещение популярности), включение — после ≥30 ночных прогонов; (2) контракт-тест плагина застрял на 26 инструментах — актуализирован до 35; (3) документация README/ARCHITECTURE/HERMES_INTEGRATION/SYNC.md/skills/ob2h была на уровне v0.9 — актуализирована; (4) отступление от §3 v1.3: `tests/fixtures/golden_synthetic.jsonl` не создавался — метрики bench тестируются инлайн в test_bench.rs (фикстура не нужна); (5) формулировка «M6 = схема v7» исправлена на v6 (v7 занята M7 из v1.4); (6) DoD v1.3/v1.4 закрыты по фактам (кроме владельческих: 28.5/28.6, неделя ночных прогонов, live-verify sync-пира); (7) fresh full (623 МБ) + quick (0.45 МБ) бэкапы после квантования, `backup verify` зелёные; (8) `.mimosa/`, `target_bench/` в .gitignore. Слияние release/v1.4 → main, тег v1.4.0, GitHub Release | данные bench вместо формулы «по плану»; честные чекбоксы |
| 2026-09-18 | MCP-only AST-скан: агент мог инициировать построение AST-базы только гибридно — `project_scan` через MCP падал по 60-с таймауту MCP-клиента на больших репо, и скилл учил досматривать результат через CLI/логи/БД. Вынесено в фоновый job (`src/project/scanjob.rs`: scan → god nodes → векторизация, spawn_blocking); `project_scan` ждёт до 45 с (`wait=false` — мгновенный ack), новый MCP-инструмент **`project_scan_status`** (36-й) отдаёт job-статус и сводку из БД — CLI больше не нужен агенту. Повторный запуск по идущему скану идемпотентен. CLI `project scan` не тронут (скриптовые запуски). Ветка feat/v1.4 не содержала линию релиза v1.4.0 (trust-флаг, актуализация доков) — тег v1.4.0 вмержен перед релизом 1.5.0 | контракт +1 инструмент (36), CHANGELOG; менеджер jobs — in-process (жизнь = процесс сервера), персистентность не нужна: статус нужен только во время скана |
