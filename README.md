...
2. 🏛️ **Строить детерминированный AST-граф кода (вдохновлено Graphify)**: мгновенно сканирует кодовую базу (Rust, Python, TS/JS, Go, SQL, PHP, Dart, Java, C/C++), извлекает классы, функции, интерфейсы, трейты/миксины и связи со 100% точностью и без расхода токенов LLM.
...
4. 💥 **Оценивать безопасность рефакторинга (Blast Radius / `project_impact`)**: находит все зависимые функции/файлы и оценивает уровень риска изменений (`Low`, `Medium`, `High`).
5. 🔍 **Искать код естественным языком**: гибридный семантический поиск по кодовому графу (RRF $k=60$ + локальный Candle MiniLM 384d).
6. 📁 **Работать с нулевой настройкой (Zero-Config)**: автоматически определяет корень репозитория, уважает правила `.gitignore` и привязывает сессию.
7. ⚡ **Реагировать на изменения на лету (File Watcher + Git hooks)**: автоматически обновляет граф при сохранении файлов в IDE.
...
Откройте вашего агента — все **36 инструментов** MCP (память, граф знаний, код, дриминг, Ralph-циклы) активируются автоматически!
...
### Шаг 3.6. Автоматическая настройка для других агентов (1 клик)

```bash
ob2h agent install --agent all
ob2h doctor --fix
```
Команда автоматически обнаружит и сконфигурирует Claude Code, Cursor, Windsurf, ZCode, Gemini/Antigravity, Qwen Code и OpenCode!
...
# Глубокая диагностика всех систем, баз данных, моделей и агентов
ob2h doctor
ob2h doctor --fix   # автоисправление конфигов агентов и Git-хуков

# Мультиагентное управление (Claude, Cursor, Windsurf, ZCode, Gemini, Qwen, OpenCode)
ob2h agent install --agent all
ob2h agent status

# Управление проектами и AST-сканирование кодовой базы
ob2h project list
