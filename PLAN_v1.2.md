# 🚀 План разработки OB2H v1.2: Автоматизация, Автодетект проектов и Интеллектуальный Граф
...
- [x] **20.1** Анализ радиуса изменений (Blast Radius / `project_impact`):
...
- [x] **20.2** Детектор циклических зависимостей (`find_circular_dependencies`):
...
- [x] **20.3** Метрики связанности и стабильности пакетов:
...
- [x] Все юнит- и интеграционные тесты Rust проходят (`cargo test`), число тестов $\ge 50$. (50 тестов пройдено)
- [x] Линтер `cargo clippy --all-targets` проходит без ошибок.
- [x] При запуске `ob2h serve` в любом репозитории агент сразу видит контекст проекта без ручного `project_init`.
- [x] Повторный скан проекта на 1 000+ файлов занимает меньше 30 мс благодаря `project_files` SHA256-кэшу.
- [x] Корректно отрабатывают правила `.gitignore` без замусоривания графа артефактами сборки.
- [x] Работает фоновый File Watcher и обновляет граф при сохранении файлов.
- [x] Команда `ob2h doctor` успешно валидирует окружение и находит все поддерживаемые агенты.
- [x] Документация (`README.md`, `CHANGELOG.md`, `docs/ARCHITECTURE.md`) обновлена.
