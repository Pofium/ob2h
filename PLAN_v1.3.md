...
как **M6** (версия схемы v6; формулировка «v7» в ранних ревизиях плана была
ошибочна — схема v7 занята миграцией M7 из PLAN_v1.4, soft-delete рёбер) — см. Фазу 26.
...
из этого плана) — в репозитории выполняется как **M6** (версия схемы v6).
...
- [x] **28.1** Dream-фаза 3 (FR-K7, за autodream-гейтами): ревизия stale-findings по текущему
  AST/спеке → verified (source=dream) или подтверждение stale/decay; `OB2H_DREAM_RALPH=false`.
- [x] **28.2** Скилл `ralph-loop` (FR-K8): черновик — Приложение C спеки; деплой
  `ob2h skill install` во все harness; правила вердиктов (red tests ≠ готово; лестница минимальности).
  *(исходник `skills/ralph-loop/SKILL.md` готов; деплой в 16 harness и мини-цикл в чате — шаги
  владельца из приёмки §11)*
- [x] **28.3** doctor (диагностика M6, сироты-apply-runs), `omnes_backup` (новые таблицы
  автоматически), `omnes_stats` (блок ralph: runs/iterations/findings by verdict),
  CLI `ob2h ralph findings-to-memory` (FR-K12).
- [x] **28.4** `ralph_*`/`ast_changes` вне синк-бандлов (ADR-K6); SYNC.md дополнить.
- [ ] **28.5** Benchmark-пакет (FR-K10, §8 спеки): fixture-репо, методика «с ob2h-знаниями / без»,
  ...
  *(методика зафиксирована в спеке §8; fixture + агенты-прогоны — отдельная задача с e2e владельца)*
- [ ] **28.6** Приёмка спеки §11 (полный чек-лист) + кириллический путь проекта (NFR-K4).
  *(автоматизируемая часть закрыта Ф26–27 + 28.1–28.4; живой мини-цикл в Hermes/ZCode — владелец)*
