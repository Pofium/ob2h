...
- [ ] **21.1** CLI `ob2h bench [--mode search|context] [--k 5,10] [--json]`:
...
- [ ] **21.2** Наполнение: 30–40 запросов по живой памяти — разговорные русские формулировки
  («как чинить кириллицу в коннекторах», «куда едет сторе VPS»), синонимы, опечатки;
  `expect_keys` — реальные ключи `hmem-*`.
- [ ] **21.3** Детерминизм в тестах: bench на фиксированных векторах-заглушках (§7 AGENTS.md),
  без сети и без fastembed; реальный прогон — вручную на живой БД.
- [ ] **21.4** Фиксация baseline: результат bench до любых изменений — в `docs/bench_baseline.md`.
...
- [ ] **22.1** Гибрид в `build_context(query)` (`src/memory/service.rs`):
...
- [ ] **22.3** MMR-диверсификация (λ=0.7 по эмбеддингам): топ по важности больше не вытесняет
  другие категории пятью вариациями одного правила.
- [ ] **22.4** Бюджет блока: `OB2H_PREFETCH_MAX_CHARS` (дефолт 8000) — резать по record-границам
...
- [ ] **22.5** `touch_access` для записей, реально попавших в блок (сейчас — только из search).
- [ ] **Тесты 21→22:** recall@5/MRR нового `build_context` ≥ baseline на golden set;
  блок ≤ бюджета при 283+ записях; детерминизм при фиксированной БД; тест «нет spill»
  (размер блока < порога Hermes).
...
- [ ] **24.1** Запись эмбеддингов в формате v2 (§3) в `src/vector/` + `src/embedding/`:
  чтение старых f32-блобов без миграции, запись — всегда v2.
- [ ] **24.2** Разовая конвертация: `ob2h db quantize-embeddings [--dry-run]` — батчевое
...
- [ ] **24.3** `omnes_backup --scope full|quick` (дефолт `full` — как сейчас):
