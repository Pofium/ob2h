...
- [ ] **35.2** Решение по реранкеру (25.3): после ≥ 30 ночных прогонов (30.3) — если
  ...
  **СТАТУС 35.2 (13.09.2026): решить нельзя — данных нет.** `data/bench/history.jsonl`
  отсутствует (0 из ≥ 30 ночных прогонов гейта), а флага/профиля `OB2H_RERANK` в коде
  нет вовсе (grep по `src/` — 0 совпадений; есть только проектная запись в плане).
  Дефолт rerank'а остаётся off; решение фиксируется, когда ночной гейт накопит ≥ 30
  прогонов с rerank-профилем. Это честный итог: цифр, на которых можно перевернуть
