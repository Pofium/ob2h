...
- [x] **31.1** Дешёвый save-time пре-чек (`memory_save`, без LLM): топ-1 косинусный сосед.
  ...
  *Реализация: пороги 0.98/0.75 (`COS_IDENTITY`/`COS_SUSPECT`); вилка подозрения
  продлена до 0.98 без пропуска; state-update отфильтровывается LLM-вердиктом 31.2
  (supersedes). Свежая формулировка вытесняет старую (content = новый).*
...
- [x] **31.4** MCP-инструмент **`memory_merge` (№34)**: `memory_merge(keys[], canonical_key?, note?)`
  ...
  в отчёте. *Единый движок `MemoryService::merge_records` — им же пользуется дрим (31.2);
  контракту инструментов добавлен №34.*
...
- [x] **Тесты:** детерминированный пре-чек на фиксированных векторах (0.98/0.95/0.75
  ...
  *(tests/test_consolidation.rs — 9 тестов, включая save-time 31.1 на ScriptedEmbedding)*
...
- [x] **33.1** Переиспользование `src/graph/pagerank.rs` (Ф29 v1.3): память как отдельный
  ...
  **edge-type-aware** веса в конфиге `OB2H_PPR_WEIGHTS` — формат JSON `{"kind": weight}`,
  ...
  на записи-хабе.
  *(Движок `src/graph/pagerank.rs` написан с нуля (29.1 v1.3): итеративный PPR, damping
  клампится 0.5–0.85, ≤20 итераций, L1-сходимость, degree-normalization, висячие узлы
  возвращают массу в персонализацию. Память: `MemoryService::ppr_rank` — узлы = живые
  записи (топ-N по importance/recency), рёбра = `memory_links` с весом kind × вес ребра;
  entity-фразы дают сид с весом 0.5; `normalize_entity` — 33.4. Конфиг:
  `OB2H_PPR_WEIGHTS`, `OB2H_PPR_DAMPING` (0.85).)*
- [x] **33.2** `graph_reason(query, project_id?, scope?)` — опциональный
  ...
  *(В памяти: `MemoryService::ppr_context` — сиды из гибридных хитов, факт-блок
  `Память (PPR-подграф)`, уверенность = усреднение trust по PPR-массе (PPR сворачивает
  веса путей в скор), лимиты `OB2H_GRAPH_REASON_MEMORY_MAX_NODES=500` /
  `OB2H_GRAPH_REASON_MEMORY_TIMEOUT_MS=1000` с урезанием до топ-3 и флагом truncated.
  Отсутствие scope — прежний ответ по графу знаний (регресс-тест); `all` — граф знаний
  + PPR-блок памяти.)*
- [x] **33.3** `memory_search` mode=graph: PPR-расширение вместо 1-hop (23.5) при
  `related=true`; fallback на 1-hop, если PPR недоступен/не построен.
  *(mode=graph включает блок `[ppr]` и без `related=true`; при пустом/ошибочном PPR —
  откат на `related_records` (1-hop). Сиды исключаются из расширения — как в 1-hop.)*
- [x] **33.4** Нормализация сущностей при построении рёбер: lowercase, ё→е, схлопывание
  инициалов/форм — меньше шумовых same_project/entity-рёбер.
  *( `normalize_entity`: lowercase, ё→е, пунктуация → пробел, отброс однобуквенных
  инициалов, склейка дефисных форм; применяется к токенам ключей при сидировании.)*
- [x] **Тесты:** синтетическая цепочка A→B→C по memory_links — запрос по A достигает C
...
- [x] **35.1** Замер p95 `memory_search` на живой БД после квантования (Ф24) — в
  ...
  **ИТОГ 35.1 (13.09.2026, копия живой БД 605 415 узлов, release):** гейт превышен —
  `memories` p95 86 мс (284 вектора), `graph_nodes` p95 2344 мс, эмбеддинг запроса
  p95 80 мс (пол). Эксперимент проведён: vec0-индекс (`src/vector/vec0.rs`,
  `sqlite-vec 0.1.9`, режимы int8/bit, флаг `OB2H_VEC0`, `ob2h vec0 build|stats|recall`).
  **recall@10 = 1.0000** (критерий ≥ 0.99 ✓), векторный скан 566 → 192 мс (**2.9×**,
  ориентир §8 «≈2.6×» подтверждён), но end-to-end p95 2408 → 2093 мс: доминирует не
  вектор, а **лексический полный скан** `graph_nodes` (~1.8 с из 2.34 с). Критерий
  «p95 < 50 мс» НЕ достигнут → флаг оставлен opt-in (дефолт off), DiskANN не нужен,
  следующий шаг (вне 35.1) — FTS5 для лексической стадии. Полный разбор и цифры:
  **`docs/ADR-35.1-sqlite-vec.md`**; latency-секция — `docs/bench_baseline.md`.
- [x] **35.2** Решение по реранкеру (25.3): после ≥ 30 ночных прогонов (30.3) — если
  ...
  **РЕАЛИЗАЦИЯ (13.09.2026):** механизм накопительной статистики готов:
  `src/cli/bench_history.rs` — каждый bench-прогон дописывает `data/bench/history.jsonl`
  в формате §4 (`ts, mode, recall@5/@10, mrr, p95_ms, db_size_mb, embedding_backend,
  vec0, dream_sha`); `ob2h bench --mode history` — сводка: счётчики, медианы p95
  off/on (OB2H_VEC0), вердикт 35.2 (<30 прогонов → отложено; rerank-профиля нет →
  OB2H_RERANK остаётся off, честно — 25.3 не реализован, остаётся бэклогом; ≥10
  latency-прогонов → сравнение vec0 с порогом 30%); ночной прогон — сервер (serve)
  раз в 24 ч гонит latency-бенч по golden-набору и дописывает history (best-effort,
  `OB2H_BENCH_NIGHTLY=0` выключает). Тесты `tests/test_bench_history.rs` (2):
  append+analyze (медианы, счётчики, вердикт), отсутствие history — честное сообщение.
  Сам вердикт по реранкеру автоматически не принимается — нужен 25.3 (rerank-профиль);
  решение по OB2H_VEC0 владельцем по данным `bench --mode history` (ADR-35.1).

  **СТАТУС 35.2 (обновлён 13.09.2026, вечер): механизм реализован** — history.jsonl
  пишется каждым прогоном, ночные прогоны запускает сервер, сводка/вердикт:
  `ob2h bench --mode history`. Решение по реранкеру по-прежнему требует 25.3
  (rerank-профиль — бэклог); решение по OB2H_VEC0 — за владельцем после ≥30 ночей
  (сейчас 0/30, счётчик начнёт расти с развёртыванием нового бинарника).
- [x] **35.3** MCP tool annotations (бэклог §8 v1.3): `readOnlyHint` на read-only инструменты
...
- [x] **37.1** `project_context(id?, query?, max_tokens?, mode?)`: `mode=repo_map` —
...
- [x] **37.2** Дефолтный `project_context` без mode — прежнее поведение (совместимость).
- [x] **Тесты:** на fixture-репо карта ≤ бюджета в ≥ 95% прогонов; God Nodes попадают
  раньше хвостов; детерминизм при той же БД.

  **РЕАЛИЗАЦИЯ (13.09.2026):** `src/graph/repomap.rs` — file-dependency граф из
  прямых file→file IMPORTS + ко-импортной связности (пара файлов с общим
  ExternalModule получает ребро 1/|файлы модуля| — TF-IDF-подобный сигнал,
  компенсирует отсутствие CALLS-рёбер); PPR (pagerank.rs Ф33) с сидами из
  query-LIKE или всех файлов; блоки «## файл (ppr) → сигнатуры из описаний
  AST-сканера»; бинарный поиск максимального префикса под бюджет (~4 симв/токен,
  резерв под служебный хвост). `project_context` += mode/max_tokens (дефолт
  context — прежний формат), CLI `ob2h project repo-map --id --tokens --query`.
  Тесты `tests/test_repomap.rs` (4): God Node раньше хвоста, 6 бюджетов 8→8192tok
  все ≤ бюджета (детерминизм → 100% > 95%), query-фокус (изолированный файл без
  PPR-массы честно выпадает), детерминизм, пустой проект. E2E на живой копии
  (ob2h, 91 файл): sync/mod.rs — топ-хаб, query=vec0 поднимает vec0.rs (ppr 0.10),
  853/1024 и 3816/4096 ток (утилизация 83–93%).
